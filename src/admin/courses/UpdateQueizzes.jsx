...
export default function UpdateQuizes() {
    ...
    const getQuizById = async () => {
        const response = await QuizesRequests.getQuizById(quizId, Token['Al-Azhar']);
        return response;
    }

    const { data: quizData, isLoading: quizDataLoading, refetch: refetchQuiz } = useQuery({
        queryKey: ["get quiz by id", quizId],
        queryFn: getQuizById,
        enabled: !!quizId && !!Token['Al-Azhar']
    });
    ...
    const handleUpdateQuiz = async (values) => {
        const neededData = {
            ...
            courseId,
            oldVideoId: quizData?.quiz?.videoId
        }

        console.log(neededData);

        const response = await QuizesRequests.updateQuiz(quizId, neededData, Token['Al-Azhar']);
        if (response?.success) {
    ...
    const { data: availableVideosForQuiz, isLoading: availableVideosForQuizLoading } = useQuery({
        queryKey: ["available videos for quiz update"],
        queryFn: getAvailableVideosForQuiz,
        enabled: !!Token['Al-Azhar']
    });


    if (courseMainDataLoading || quizDataLoading || availableVideosForQuizLoading) return <Loader />;
                ...
                <motion.div className="mb-4 sm:mb-6" variants={itemVariants}>
                    <div className="w-full p-2 sm:p-3">
                        <div className="flex flex-wrap items-center text-xs sm:text-sm">
                            ...
                            <Link to={`/admin/course-videos/${courseId}`} className='text-gray-500 hover:text-gray-700 truncate max-w-[150px] 
                            pl-2 sm:max-w-none'>{courseMainData?.title} / </Link>
                            <span className='text-blue-500 font-bold px-2'>Update Quiz {quizData?.data?.quiz?.title}</span>
                        </div>
                    </div>
                </motion.div>
